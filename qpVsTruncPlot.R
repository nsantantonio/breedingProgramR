formatPop <- function(popL, depth = 1, meanVariety = TRUE, removeErr = TRUE){
	...
	list(nVar = nVar, cyclePerYr = cyclePerYr, RGSCyr = RGSCyr, RGSCgen = RGSCgen, yr = yr, xlims = xlims, ylims = ylims, 
		 varL = varL, Vg = Vg, gsL = gsL, RGSCacc = RGSCacc, SL = SL, iL = iL, factorNames = factorNames)
}
...
# truncVSquadprog30yr1000QTL_quadprog_fin0.005_fout0.2_truth2_rgsc0.2_vdp40x200  
# truncVSquadprog30yr1000QTL_trunc_truth1_rgsc0.2_vdp10x50
# truncVSquadprog30yr1000QTL
# i = "1000QTL"
...
defArgs <- list(factors = list(QTL = c("1000QTL"),#, "100QTL", "10QTL"),
							   select = c("trunc", "quadprog_fin0.005_fout0.2", "quadprog_fin0.01_fout0.2"),
							   truth = c("truth0", "truth1", "truth2"),
							   crossSel = c("rgsc0.2"), 
							   VDPint = c("vdp4x25", "vdp10x50", "vdp20x100", "vdp40x200")
							   ), 
				simName = "truncVSquadprog30yr", 
...
for(i in factors$QTL){
	for(j in factors$select){
		for(k in factors$truth){
			for(l in factors$crossSel){
				for(m in factors$VDPint){
					# simNameijklm <- paste0(c(simName, c(i, j, k, l, m)), c("", "", "_", "", "_", ""), collapse = "")
					simNameijklm <- paste0(c(simName, c(i, j, k, l, m)), c("", "_", "_", "_", "_", ""), collapse = "")
					# simNameijklm <- paste0(c(simName, c(i, j, k, l)), collapse = "_")
					load(paste0(parDir, "/results/", projDir, simNameijklm, "/", simNameijklm, ".RData"))
					# sel <- if(j == "") "trunc" else j
					if(byVDP) popList[[i]][[j]][[k]][[m]][[l]] <- simrun else popList[[i]][[j]][[k]][[l]][[m]] <- simrun
...
popList <- rlapply(popList, level = 5, f = unlist, recursive = FALSE)
popL <- popList

# popList[[1]][[1]][[1]][[1]][[1]][[1]][["paramL"]][["nFam"]]
# parm <- rlapply(popList, level = 6, f= function(x) {x[["paramL"]][["nFam"]]}, combine = c)
...
for(j in names(statList$varL[[1]][[1]])){ # truth0  truth1 truth2
	for(k in names(statList$varL[[1]][[1]][[1]])){ # VDP
		
		pdfName <- paste0(parDir, "/figures/", paste(c("truncVSquadprog", i, j, k), collapse = "_"), ".pdf") 
		pdf(pdfName, width = 12, height = 7)
		
		ylims <- range(c(statList$varL[[1]][["trunc"]][[j]][[k]], statList$varL[[1]][["quadprog_fin0.01_fout0.2"]][[j]][[k]], statList$varL[[1]][["quadprog_fin0.005_fout0.2"]][[j]][[k]])) * c(0.9, 1.1)
		plot(statList$RGSCyr, statList$varL[[1]][["trunc"]][[j]][[k]], type = "l", ylim = ylims, xlab = "year", ylab = "Variety Mean", main = "Variety Means", xaxt = "n")
	    axis(1, at = c(0, statList$RGSCyr), labels = c(0, statList$yr))
		lines(statList$RGSCyr, statList$varL[[1]][["quadprog_fin0.005_fout0.2"]][[j]][[k]], lty = 2)
		lines(statList$RGSCyr, statList$varL[[1]][["quadprog_fin0.01_fout0.2"]][[j]][[k]], lty = 3)
		legend("topleft", legend = c("trunc", "fin0.005", "fin0.01"), lty = 1:3)


		ylims <- range(c(statList$gsL[[1]][["trunc"]][[j]][[k]], statList$gsL[[1]][["quadprog_fin0.01_fout0.2"]][[j]][[k]], statList$gsL[[1]][["quadprog_fin0.005_fout0.2"]][[j]][[k]])) * c(0.9, 1.1)
		plot(statList$RGSCgen, statList$gsL[[1]][["trunc"]][[j]][[k]], type = "l", ylim = ylims, xlab = "year", ylab = "Recurrent Population Mean", main = "Recurrent Population Mean", xaxt = "n")
	    axis(1, at = c(0, statList$RGSCyr), labels = c(0, statList$yr))
		lines(statList$RGSCgen, statList$gsL[[1]][["quadprog_fin0.005_fout0.2"]][[j]][[k]], lty = 2)
		lines(statList$RGSCgen, statList$gsL[[1]][["quadprog_fin0.01_fout0.2"]][[j]][[k]], lty = 3)
		legend("topleft", legend = c("trunc", "fin0.005", "fin0.01"), lty = 1:3)

		ylims <- range(c(statList$Vg[[1]][["trunc"]][[j]][[k]], statList$Vg[[1]][["quadprog_fin0.01_fout0.2"]][[j]][[k]], statList$Vg[[1]][["quadprog_fin0.005_fout0.2"]][[j]][[k]])) * c(0.9, 1.1)
		plot(statList$RGSCgen, statList$Vg[[1]][["trunc"]][[j]][[k]], type = "l", ylim = ylims, xlab = "year", ylab = "Recurrent Population Variance", main = "Variance", xaxt = "n")
	    axis(1, at = c(0, statList$RGSCyr), labels = c(0, statList$yr))
		lines(statList$RGSCgen, statList$Vg[[1]][["quadprog_fin0.005_fout0.2"]][[j]][[k]], lty = 2)
		lines(statList$RGSCgen, statList$Vg[[1]][["quadprog_fin0.01_fout0.2"]][[j]][[k]], lty = 3)
		legend("topleft", legend = c("trunc", "fin0.005", "fin0.01"), lty = 1:3)

		ylims <- c(0, 1)
		plot(statList$RGSCgen[-length(statList$RGSCgen)], statList$RGSCacc[[1]][["trunc"]][[j]][[k]], type = "l", ylim = ylims, xlab = "year", ylab = "Recurrent Population Prediction Accuracy", main = "Prediction Accuracy", xaxt = "n")
	    axis(1, at = c(0, statList$RGSCyr), labels = c(0, statList$yr))
		lines(statList$RGSCgen[-length(statList$RGSCgen)], statList$RGSCacc[[1]][["quadprog_fin0.005_fout0.2"]][[j]][[k]], lty = 2)
		lines(statList$RGSCgen[-length(statList$RGSCgen)], statList$RGSCacc[[1]][["quadprog_fin0.01_fout0.2"]][[j]][[k]], lty = 3)
		legend("topright", legend = c("trunc", "fin0.005", "fin0.01"), lty = 1:3)
		dev.off()	
	}
}
