...
#' @param switchGSfunc integer. Used to determin when to switch from RRBLUP to RRBLUP2 when the number of individuals exceeds number of markers * switchGSfunc. Default is 4.
#' @return [value]
...
#' founderSamples = NULL, # vector of length famSize. Which individuals in the founder population should be sampled. Used when repeating simulations with different starting populations
...
#' nNuclear = 100, # integer. RCRS population size. [should modify to let change through time?]
...
#' selectTrials = c(0.5, 0.2, 0.5, 0.4), # numeric. Selection intensity within each trial. Valid between 0 and 1.
...
#' @examples none
#' @export
simSingleTraitInbred <- function(founderPop, paramL, simParam = SP, returnFunc = identity, verbose = TRUE, checkParam = TRUE, GSfunc = NULL, switchGSfunc = 4, ...){
# k = 1; paramL = userArgs; simParam <- SP; intAcross = 0.5; intWithin = 0.2; verbose = TRUE; checkParam = FALSE; GSfunc = RRBLUP; nGenOut = NULL; nGenInbr = NULL; returnFunc = getPopStats
	...
	if (checkParam){
		paramNames <- c("maxIter", "lgen", "useTruth", "traditional", "intAcross", "intWithin", "founderSamples", "founderh2", "founderBurnIn", 
						...
						"useInbreed", "useVDP", "returnVDPcrit", "selFuncOut", "selFuncIn", "inbreedFunc", "withinFamInt", "setXint", "skip", 
		...
		if (length(xtraParams)) {
			warning("Unrecognized parameters have been provided to the 'paramL' argument. If these were meant to be passed to user defined functions, please provide them directly to simSingleTraitInbred().The following will be ignored:\n", paste(xtraParams, collapse = ", "), "\n")
		}
