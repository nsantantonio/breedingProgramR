...
# i commented this out, not sure it needs to be set?
	...
	for(i in 1:(nYr + length(trials))){
		...
		if(!trials[1] %in% skip) VDP[[trials[1]]][[gen(i)]] <- setPheno(VDP[[trials[1]]][[gen(i)]], varE = h2toVe(h2hr, Vg), reps = 1)
		...
		while(ii >= max(1, i - nTrial)){
			...
			for(g in index) {
				...
				VDP[[tip1]][[gen(ii)]] <- selectInd(VDP[["headrow"]][[gen(ii)]], nInd = selOutOfHR, trait = 1, use = sel, returnPop = TRUE)
